      ...
      <div className={`p-6 rounded-2xl border ${
        ...
        <div className="overflow-x-auto text-xs">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 text-slate-405 font-bold">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-850">
              {state.substituteAssignments.length === 0 ? (
                <tr>
                  <td colSpan={6} className="p-8 text-center text-slate-400 text-xs">No historical coverages logged in DB.</td>
                </tr>
              ) : (
                state.substituteAssignments.map((sub) => (
                  <tr key={sub.id} className="hover:bg-slate-55/30 transition-colors">
                    <td className="p-3 font-medium text-slate-600">{sub.date}</td>
